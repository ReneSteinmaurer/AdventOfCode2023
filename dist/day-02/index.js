import fs from "fs";
function getGameValueFromString(str) {
...
function getPossibleGames(games, blue, green, red) {
    let possibleGames = [];
    let isPossible = true;
...
function getSumOfIdsFromPossibleGames(games) {
    let ids = 0;
    for (const game of games) {
        ids += game.id;
    }
...
function getResultOfCubesWithHighestRedGreenBlue(game) {
    let maxRed = 0;
    let maxGreen = 0;
    let maxBlue = 0;
...
function getResultOfAllWithHighestRedGreenBlue(games) {
    let result = 0;
    for (const game of games) {
        debugger;
